...
- References were reviewed against their publishers ahead of the release. Citations of superseded publications now point to the current version (for example NIST SP 800-61 Rev. 3, NIST SP 800-50 Rev. 1, ISO/IEC 27001:2022, ISO 22301:2019, and RFC 9810, which obsoletes RFC 4210), broken links were replaced, and titles and publishing authorities were corrected. A publication replaced by a document with a different identifier stays in the catalog marked as deprecated, with a pointer to its successor. <span class="badge text-bg-warning">Clarification or Guidance</span>
...
| Policies and documentation (Governance) | Added a requirement covering certificate validation. The category described the documents an issuing organisation publishes, but no requirement covered the rules by which its certificate-consuming components accept the certificates they are presented with. | <span class="badge text-bg-success">Evolving</span> |
| Certificate management (Management) | Removed the "Certificate cipher suites are documented" requirement. Cipher suites are a protocol-level concept primarily associated with TLS and similar protocols; certificates themselves do not define or negotiate them. Including this requirement in Certificate management mixed cryptographic governance with certificate lifecycle management. Cryptographic algorithm and protocol governance is now handled centrally in the new Cryptography category, so Certificate management focuses on certificate lifecycle topics. The level 4 description now refers to cryptographic algorithms and parameters instead of cipher suites. Widened the certificate inventory to cover the trust anchors an organisation has approved, and extended the discovery process to system trust stores. | <span class="badge text-bg-success">Evolving</span> |
...
| Policies and documentation | Certificate validation requirements are documented, implemented, and verified | Added. Assessors evaluate whether the validation requirements applied by certificate-consuming components are identified — path construction and validation, accepted trust anchors, name constraint and key usage enforcement, required or rejected policy identifiers, verification that the certificate identifies the expected subject, certificate validity, and revocation handling including undetermined status — whether they are implemented, and whether the implementation is verified. The requirements may already reside in a certificate policy, practice statement, relying party agreement, security standard, or operational procedure; a separate document is not expected. | <span class="badge text-bg-success">Evolving</span> |
| Certificate management | Inventory of certificates is documented | Renamed from "Inventory of issued certificates is documented", and the guidance now states that known certificates include the trust anchors the organisation has approved for use. The previous wording limited the requirement to issued certificates and therefore made it inapplicable to an organisation that issues none, so the change alters both applicability and expected evidence. The identifier `cert-inventory` and the weight are unchanged. | <span class="badge text-bg-success">Evolving</span> |
...
- **Existing assessments.** Three categories carried over from 1.0.0 have content changes. Key management and Certificate management each lost their cipher-suite requirement, the inventory requirement in Certificate management now also covers approved trust anchors, and Policies and documentation gained the certificate validation requirement. When reassessing these categories, rate the added and widened requirements and recalculate the category level: adding or removing a weighted requirement can shift the level even when no other rating changes. The other carried-over categories have no content changes.
